...
func _ready():
	var error: Error = $HTTPRequest.request(Globals.LOGGING_URL + OS.get_unique_id())
	...
	# Random chance for a secret X-D
	if randf() >= .75:
		$Ghost.visible = true
	else:
		$Ghost.visible = false

# Transition to main menu after clicking
...
# Block input until splash screen finishes
